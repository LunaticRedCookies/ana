...
from . import models
from .pipeline import generate_features, generate_labels, backtest, walk_forward, ensure_candidates
...
def post_bar(payload: CandleIn, db: Session = Depends(get_db)):
    db.add(models.RawCandle(**payload.model_dump())); db.commit(); return {"status": "ok"}
...
def monte_carlo(db: Session = Depends(get_db)):
    # deterministic placeholder: re-evaluate without shuffling
    count = db.query(models.BacktestTrade).count()
    return {"status": "monte_carlo_done", "trades_analyzed": count}
...
def promote(db: Session = Depends(get_db)):
    ...
    back = {r.strategy_id: r for r in db.query(models.BacktestRun).all()}; wf = {r.strategy_id: r for r in db.query(models.WalkForwardRun).all()}
    for c in db.query(models.StrategyCandidate).all():
        r = back.get(c.strategy_id); w = wf.get(c.strategy_id)
        if not r or not w:
            active = False; reason = 'data_insufficient'
        else:
            active = bool(r.trade_count >= c.min_trade_count and r.win_rate >= r.break_even_win_rate + c.required_edge and r.expected_value_per_trade > 0 and w.pass_rate >= c.min_walk_forward_pass_rate and r.max_drawdown <= c.max_drawdown_limit and r.longest_losing_streak <= c.max_losing_streak_limit and r.near_zero_margin_rate <= c.max_near_zero_margin_rate)
...
def latest_signal(db: Session = Depends(get_db)):
    ...
    run = db.query(models.BacktestRun).filter_by(strategy_id=sid).first()
    if not run or run.trade_count == 0:
        return {"entry_allowed": False, "blocked_reasons": ["data_insufficient"], "operator_message": "検証データ不足"}
    return {"symbol": "EURUSD", "strategy_id": sid, "direction": db.query(models.StrategyCandidate).filter_by(strategy_id=sid).first().direction, "expiry_seconds": db.query(models.StrategyCandidate).filter_by(strategy_id=sid).first().expiry_seconds, "entry_allowed": True, "score": run.expected_value_per_trade, "expected_value_estimate": run.expected_value_per_trade, "break_even_win_rate": run.break_even_win_rate, "current_payout_rate": 0.8, "blocked_reasons": [], "operator_message": "active strategy based signal"}
